...
def pm_forces(positions, mesh_shape=None, delta_k=None, halo_size=16):
    ...
    forces = [
        dops.cic_read(dops.reshape_dense_to_split(dops.ifft3d(f)), positions, halo_size)
        for f in forces_k
    ]
...
def lpt(cosmo, initial_conditions, positions, a):
    ...
    p = dops.scalar_multiply(
        dx, a**2 * growth_rate(cosmo, a) * jnp.sqrt(jc.background.Esqr(cosmo, a))
    )
    return dx, p
...
def make_ode_fn(mesh_shape):
    ...
    def nbody_ode(state, a, cosmo):
        ...
        dpos = dops.scalar_multiply(
            vel, 1.0 / (a**3 * jnp.sqrt(jc.background.Esqr(cosmo, a)))
        )

        # Computes the update of velocity (kick)
        dvel = dops.scalar_multiply(
            forces, 1.0 / (a**2 * jnp.sqrt(jc.background.Esqr(cosmo, a)))
        )
