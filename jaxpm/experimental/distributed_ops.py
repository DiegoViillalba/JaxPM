...
@partial(
    xmap,
    in_axes=({0: "x", 2: "y"}, {0: "x", 2: "y"}, {0: "x", 2: "y"}),
    out_axes=({0: "x", 2: "y"}),
...
def gradient_laplace_kernel(kfield, kvec):
    kx, ky, kz = kvec
    kk = kx**2 + ky**2 + kz**2
    kernel = jnp.where(kk == 0, 1.0, 1.0 / kk)
    return (
        kfield * kernel * 1j * 1 / 6.0 * (8 * jnp.sin(ky) - jnp.sin(2 * ky)),
...
@partial(
    ...
    in_axes=([...]),
    out_axes={0: "x", 2: "y"},
    axis_sizes={"x": mesh_size["nx"], "y": mesh_size["ny"]},
    axis_resources=axis_resources,
)
def meshgrid(x, y, z):
...
def cic_paint(pos, mesh_shape, halo_size=0):
    ...
    def fn(pos):
        ...
        mesh = jnp.zeros(
            [
                mesh_shape[0] // mesh_size["nx"] + 2 * halo_size,
        ...
        mesh = paint.cic_paint(
            mesh,
            pos.reshape(-1, 3) + jnp.array([halo_size, halo_size, 0]).reshape([-1, 3]),
        )

        # Perform halo exchange
        # Halo exchange along x
        left = lax.pshuffle(
            mesh[-2 * halo_size :], perm=range(mesh_size["nx"])[::-1], axis_name="x"
        )
        right = lax.pshuffle(
            mesh[: 2 * halo_size], perm=range(mesh_size["nx"])[::-1], axis_name="x"
        )
        ...
        left = lax.pshuffle(
            mesh[:, -2 * halo_size :], perm=range(mesh_size["ny"])[::-1], axis_name="y"
        )
        right = lax.pshuffle(
            mesh[:, : 2 * halo_size], perm=range(mesh_size["ny"])[::-1], axis_name="y"
        )
...
def cic_read(mesh, pos, halo_size=0):
    ...
    def fn(mesh, pos):
        ...
        # Exchange along x
        left = lax.pshuffle(
            mesh[-halo_size:], perm=range(mesh_size["nx"])[::-1], axis_name="x"
        )
        right = lax.pshuffle(
            mesh[:halo_size], perm=range(mesh_size["nx"])[::-1], axis_name="x"
        )
        ...
        # Exchange along y
        left = lax.pshuffle(
            mesh[:, -halo_size:], perm=range(mesh_size["ny"])[::-1], axis_name="y"
        )
        right = lax.pshuffle(
            mesh[:, :halo_size], perm=range(mesh_size["ny"])[::-1], axis_name="y"
        )
...
def reshape_dense_to_split(x):
    ...
    return x.reshape(
        [
            mesh_size["nx"],
            shape[0] // mesh_size["nx"],
            mesh_size["ny"],
            shape[2] // mesh_size["ny"],
        ]
        + shape[2:]
    )
