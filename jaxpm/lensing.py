...
def density_plane(
    positions, box_shape, center, width, plane_resolution, smoothing_sigma=None
):
    ...
    weight = jnp.where(
        (d > (center - width / 2)) & (d <= (center + width / 2)), 1.0, 0.0
    )
    ...
    density_plane = density_plane / (
        (nx / plane_resolution) * (ny / plane_resolution) * (width)
    )
...
def convergence_Born(cosmo, density_planes, coords, z_source):
    ...
    for entry in density_planes:
        ...
        convergence += im * jnp.clip(1.0 - (r / r_s), 0, 1000).reshape([-1, 1, 1])
